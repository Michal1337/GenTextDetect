...
import pypickle
from ex_params import DATASETS, DATASETS_PATH, TRAINING_HISTORY_PATH, CHECKPOINTS_PATH
...
if __name__ == "__main__":
    ...
    for name, config in DATASETS.items():
        ...
        lr_results = eval_model(lr_model, X_train_s, y_train, X_val_s, y_val, X_test_s, y_test)
        ...
        pypickle.save(
            CHECKPOINTS_PATH + f"feature_based/lr_model_{name}_2.pkl",
            lr_model
        )
        ...
        rf_results = eval_model(rf_model, X_train_s, y_train, X_val_s, y_val, X_test_s, y_test)
        record = {
                "model": "rf",
                "dataset": name,
                "feature_set": 2,
        ...
        pypickle.save(
            CHECKPOINTS_PATH + f"feature_based/rf_model_{name}_2.pkl",
            rf_model
        )
        ...
        xgb_results = eval_model(xgb_model, X_train_s, y_train, X_val_s, y_val, X_test_s, y_test)
        record = {
                "model": "xgb",
                "dataset": name,
                "feature_set": 2,
        ...
        pypickle.save(
            CHECKPOINTS_PATH + f"feature_based/xgb_model_{name}_2.pkl",
            xgb_model
        )